...
Un exemple de la structure de la base de données, mais l’important, c’est d’avoir un couplage minimal entre le système et la base de données.

\begin{verbatim}
exo_nom/
 |-- config.yaml
 |-- Base1/
     |-- main.c(ou autre language)
     |-- config.yaml
     |-- Makefile(ou equivalence)
     |-- test.c
 |-- Base2/
     |-- Base3
         |-- sum.py
...
Le deroulement de l'exercice sera dans un nouvelle SHELL. Le système doit permettre de compiler le code de l'utilisateur en utilisant les paramètres définis dans le fichier de métadonnées de l'exercice. Les erreurs et avertissements de compilation doivent être affichés de manière claire et lisible.
